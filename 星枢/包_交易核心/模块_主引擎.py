...
from 星枢.包_事件引擎 import 类_事件,类_事件引擎
...
from .模块_转换器 import 类_仓位转换器
from 星枢.包_交易核心.包_国际化 import _
...
class 类_主引擎:
    ...
    def 发送订单(self, 请求: 类_订单请求, 网关名称: str) -> str:
        """委托下单"""
        网关实例: 类_基础网关 = self.获取网关(网关名称)
        return 网关实例.发送订单(请求) if 网关实例 else ""
...
class 订单管理引擎(基础引擎):
    ...
    def 绑定查询功能(self) -> None:
        ...
        self.主引擎.更新订单请求 = self.更新订单请求
        self.主引擎.转换订单请求 = self.转换订单请求
    ...
    def 处理订单事件(self, 事件: 类_事件) -> None:
        订单 = 事件.数据
        self.订单字典[订单.唯一标识] = 订单

        if 订单.是否活跃():
            self.活跃订单字典[订单.唯一标识] = 订单
        elif 订单.唯一标识 in self.活跃订单字典:
            self.活跃订单字典.pop(订单.唯一标识)
    ...
    def 处理合约事件(self, 事件: 类_事件) -> None:
        ...
        if 合约.网关名称 not in self.仓位转换器字典:
            self.仓位转换器字典[合约.网关名称] = 类_仓位转换器(self)

    def 处理报价事件(self, 事件: 类_事件) -> None:
        报价 = 事件.数据
        self.报价字典[报价.唯一标识] = 报价

        if 报价.是否活跃():
            self.活跃报价字典[报价.唯一标识] = 报价
        elif 报价.唯一标识 in self.活跃报价字典:
            self.活跃报价字典.pop(报价.唯一标识)
    ...
    def 更新订单请求(self, 请求: 类_订单请求, 订单标识: str, 网关名称: str) -> None:
    ...
    def 转换订单请求(
        ...
        转换器 = self.仓位转换器字典.get(网关名称)
        return 转换器.转换订单请求(请求, 锁定, 净额) if 转换器 else [请求]

    def 获取仓位转换器(self, 网关名称: str) -> 类_仓位转换器:
